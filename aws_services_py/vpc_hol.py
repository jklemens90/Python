...
#Added a response variable with calls the describe_vpcs API which checks to see if there's an existing VPC with the same name
...
#Added a response variable with calls the describe_IG API which checks to see if there's an existing IG with the same name
...
# Create 3 subnets in the VPC. Specify vpc, cidrblock, and the AZ. Initiate by calling the ec2.client API   
